        ...
        <ul class="sidebar-menu">
            ...
            @if(checkAdminHasPermission('course.management') || checkAdminHasPermission('course.certificate.management') || checkAdminHasPermission('badge.management') || checkAdminHasPermission('blog.view'))
                ...
                @if(checkAdminHasPermission('course.management'))
                    <li class="nav-item dropdown {{ isRoute(['admin.fee-structure-locations.*', 'admin.fee-structure-durations.*', 'admin.fee-structure-branches.*'], 'active') }}">
                        <a href="javascript:void()" class="nav-link has-dropdown"><i class="fas fa-dollar-sign"></i><span>{{ __('Fee Structure') }}</span></a>
                        <ul class="dropdown-menu">
                            <li class="{{ isRoute('admin.fee-structure-locations.*', 'active') }}">
                                <a class="nav-link" href="{{ route('admin.fee-structure-locations.index') }}">
                                    {{ __('Locations') }}
                                </a>
                            </li>
                            <li class="{{ isRoute('admin.fee-structure-durations.*', 'active') }}">
                                <a class="nav-link" href="{{ route('admin.fee-structure-durations.index') }}">
                                    {{ __('Durations') }}
                                </a>
                            </li>
                            <li class="{{ isRoute('admin.fee-structure-branches.*', 'active') }}">
                                <a class="nav-link" href="{{ route('admin.fee-structure-branches.index') }}">
                                    {{ __('Branches') }}
                                </a>
                            </li>
                        </ul>
