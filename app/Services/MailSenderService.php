            ...
            try {
                if (self::isQueable()) {
                    ...
                    if ($user_type == 'all_user') {
                        ...
                        Log::info('=== sendVerifyMailToUserFromTrait: Starting email send for single_user ===', [
                            'user_id' => $user_info->id ?? 'N/A',
                            'user_email' => $user_info->email ?? 'N/A',
                            'user_name' => $user_info->name ?? 'N/A'
                        ]);
                        
                        try {
                            Log::info('Step 1: Fetching email template "user_verification"');
                            $template = EmailTemplate::where('name', 'user_verification')->first();
                            
                            if (!$template) {
                                Log::error('ERROR: Email template "user_verification" not found!');
                                return false;
                            }
                            
                            Log::info('Step 2: Template found', [
                            ...
                            Log::info('Step 3: Replacing template variables', [
                            ...
                            Log::info('Step 4: Prepared email data', [
                                'subject' => $subject,
                                'message_length' => strlen($message),
                                'recipient_email' => $user_info->email
                            ]);
                            
                            Log::info('Step 5: Sending email via Mail::to()->send()');
                            ...
                            Log::error('=== ERROR in sendVerifyMailToUserFromTrait ===', [
                            ...
                            if (app()->isLocal()) {
                                Log::error('Local environment error details: ' . $ex->getMessage());
                            }
