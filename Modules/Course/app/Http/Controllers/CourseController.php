...
class CourseController extends Controller {
    ...
    function create() {
        ...
        return view('course::course.create', compact('instructors', 'locations'));
    }
    ...
    function editView(string $id) {
        ...
        $locations = \App\Models\FeeStructureLocation::where('status', 1)->orderBy('order')->orderBy('name')->get();
        $editMode = true;
        return view('course::course.create', compact('course', 'editMode', 'instructors', 'locations'));
    }
    ...
    public function edit(Request $request) {
        ...
        case '4':
            ...
            $existingFeeStructures = \App\Models\CourseFeeStructure::where('course_id', $courseId)->with(['location', 'duration'])->get();
            return view('course::course.course-fees', compact('course', 'courseId', 'locations', 'durations', 'existingFeeStructures'));
        case '5':
            $course = Course::findOrFail($request->id);
    ...
    function update(Request $request) {
        ...
        case '6':
            ...
            return response()->json([
                'status'   => 'success',
                'message'  => __('Updated Successfully'),
                'redirect' => $request->next_step == 6 ? route('admin.courses.index') : route('admin.courses.edit', ['id' => Session::get('course_create'), 'step' => $request->next_step]),
            ]);
